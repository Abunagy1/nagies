...
exports.author_detail = function(req, res, next) {
    async.parallel({
            ...
            author_posts: function(callback) {
                Post.find({ author: req.params.id }, 'title summary').exec(callback);
            },
        },
        function(err, results) {
            ...
            res.render('author_detail', {
                ...
                author_posts: results.author_posts,
            });
        }
    );
};
...
exports.author_create_post = [
    ...
    (req, res, next) => {
        ...
        const author = new Author({ first_name: req.body.first_name, family_name: req.body.family_name, date_of_birth: req.body.date_of_birth, date_of_death: req.body.date_of_death, });
        if (!errors.isEmpty()) {
...
exports.author_delete_get = function(req, res, next) {
    async.parallel({
        ...
        function(err, results) {
            ...
            if (results.author == null) {
                // No results.
                res.json({ success: false, error: "Author doesn't exist" }).redirect('/authors');
            }
            // Successful, so render.
    ...
    async.parallel({
        ...
        function(err, results) {
            ...
            if (results.author_posts.length > 0) {
                // Author has posts. Render in same way as for GET route.
                res.render('author_delete', { title: 'Delete Author', author: results.author, author_posts: results.author_posts, });
                return;
            } else {
                // Author has no posts. Delete object and redirect to the list of authors.
